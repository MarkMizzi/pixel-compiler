...
\begin{algorithm}
   ...
   \KwData{
      \begin{itemize}
        ...
        \item $line$ is the current line in the source being lexed.
        \item $col$ is the current column in the source being lexed.
        \item $c$ is the last character obtained from the input stream.
        \item $token$ is the result: the token lexed from an initial segment of $input$. In the pseudocode, tokens are represented by a pair, with the first element being the token type, and the second being the value (the input characters which were lexed).
        \item $state$ is the current state of the lexer.
        \item $transTable$ is the transition table of the lexer.
      \end{itemize}}
    \If{$\mathbf{isEmpty}(input)$}{
